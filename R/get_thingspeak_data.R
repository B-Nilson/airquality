...
#' @param api_key Required for private channels. Specify the Read API Key found on the API Keys tab of the channel view. (string)
#' @param results Optional. Number of entries to retrieve. Maximum is 8,000. (integer)
#' @param days Optional. Number of 24-hour periods before now to include. Default is 1. (integer)
#' @param minutes Optional. Number of 60-second periods before now to include. Default is 1440. (integer)
#' @param start Optional. Start date in format YYYY-MM-DD HH:NN:SS. (datetime)
#' @param end Optional. End date in format YYYY-MM-DD HH:NN:SS. (datetime)
#' @param timezone Optional. Identifier from Time Zones Reference. (string)
#' @param offset Optional. Timezone offset for displaying results. Use `timezone` for greater accuracy. (integer)
#' @param fields Optional. Character vector of field names to collect. (string)
#' @param status Optional. Include status updates by setting `status = TRUE`. (logical)
#' @param metadata Optional. Include channel metadata by setting `metadata = TRUE`. (logical)
#' @param location Optional. Include latitude, longitude, and elevation by setting `location = TRUE`. (logical)
#' @param min Optional. Minimum value to include. (numeric)
#' @param max Optional. Maximum value to include. (numeric)
#' @param round Optional. Round values to this many decimal places. (integer)
#' @param timescale Optional. Get first value in this many minutes. Valid values: 10, 15, 20, 30, 60, 240, 720, 1440, "daily". (integer or string)
#' @param sum Optional. Get sum over this many minutes. Valid values: 10, 15, 20, 30, 60, 240, 720, 1440, "daily". (integer or string)
#' @param average Optional. Get average over this many minutes. NaN values are treated as 0. Valid values: 10, 15, 20, 30, 60, 240, 720, 1440, "daily". (integer or string)
#' @param median Optional. Get median over this many minutes. Valid values: 10, 15, 20, 30, 60, 240, 720, 1440, "daily". (integer or string)
#'
#' @import S7
#' @export
