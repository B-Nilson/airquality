...
taylor_diagram <- function(
  ...
  if (length(data_cols) != 2) {
    stop(paste(
      "argument `data_cols` must have a length of two, not",
      length(data_cols)
    ))
  }
  if (!all(data_cols %in% names(dat))) {
    stop(paste(
      "All column names in argument `data_cols` must be found in `dat`, these ones are missing:",
      paste(data_cols[!data_cols %in% names(dat)], collapse = ", ")
    ))
  }
  if (!all(group_by %in% names(dat))) {
    stop(paste(
      "All column names in argument `group_by` must be found in `dat`, these ones are missing:",
      paste(group_by[!group_by %in% names(dat)], collapse = ", ")
    ))
  }
  if (!is.null(facet_by)) {
    if (!all(facet_by %in% names(dat))) {
      stop(paste(
        "All column names in argument `facet_by` must be found in `dat`, these ones are missing:",
        paste(facet_by[!facet_by %in% names(dat)], collapse = ", ")
      ))
    }
  }
  if (!is.null(cor_minimum)) {
    if (cor_minimum < -1 | cor_minimum > 1) {
      stop(paste(
        "argument `cor_minimum` must be between -1 and 1, not",
        cor_minimum
      ))
    }
  }
  if (rmse_minimum < 0) {
    stop(paste(
      "argument `rmse_minimum` must be greater than or equal to 0, not",
      rmse_minimum
    ))
  }
  if (!is.null(sd_maximum)) {
    if (sd_maximum <= 0) {
      stop(paste(
        "argument `sd_maximum` must be greater than 0, not",
        sd_maximum
      ))
    }
  }
...
make_taylor_diagram_template <- function(
  ...
  y_max <- ifelse(min_cor < 0, sd_max, get_y(sd_max, min_cor)) + padding_limits
      ...
      \(at) {
        observed |>
          dplyr::mutate(
            ...
            label = ifelse(
              label_type == "percent", # TODO: implement in taylor_diagram()
              paste(round(at * 100), "%"),
              round(at, 2)
            )
          )
