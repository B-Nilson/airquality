...
test_that("AQHI returns a dataframe of proper size", {
  output = AQHI(datetimes = obs$date, pm25_hourly = obs$pm25,
...
test_that("AQHI (pm25 only) returns a dataframe of proper size", {
  output = AQHI(datetimes = obs$date, pm25_hourly = obs$pm25, quiet = TRUE)
  expect_warning(AQHI(datetimes = obs$date, pm25_hourly = obs$pm25))
  expect_s3_class(output, "data.frame")
